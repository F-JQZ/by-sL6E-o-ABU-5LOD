...
def error_embed(msg: str) -> discord.Embed:
    e = discord.Embed(title="FiveM Bot", description=msg, color=COLOR_ERROR)
...
class SearchIDModal(discord.ui.Modal, title="🔍 بحث بـ Server ID"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="🟠 Steam",           value=f"`{extract_identifier(identifiers,'steam:')}`"   or "`—`",    inline=True)
        embed.add_field(name="🔵 Discord",         value=f"`{extract_identifier(identifiers,'discord:')}`" or "`—`",    inline=True)
        embed.add_field(name="🔑 License",         value=f"`{extract_identifier(identifiers,'license:')}`" or "`—`",    inline=True)
...
class PanelView(discord.ui.View):
    ...
    async def btn_info(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title="FiveM Bot", description="**ℹ️ دليل الاستخدام**", color=COLOR_DEFAULT)
